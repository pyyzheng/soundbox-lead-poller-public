...
SUBOFFICE_ASSIGNEE = "fldojCR4c6"
AGENT_ASSIGNEE = "fld8G1tcDd"
QUEUE_ASSIGNEE = "fldYadBK0j"
...
def public_pool_or_same_dept() -> str:
    """公区（旧本部公共 / 南美非洲公区）允许跨部门继承历史负责人。"""
    r = ref
    return (
        f"OR({r('flde1hdT3S')}=\"本部（公共）\","
        f"{r('flde1hdT3S')}=\"南美非洲公区\","
...
def build_assignment_source() -> str:
    ...
    return (
        ...
        f"{r('fldyMGTE7A')}=\"是\",{public_pool_or_same_dept()}),\"查重命中\","
...
def build_dup_inherit_reason() -> str:
    ...
    return (
        ...
        f'IF(AND({r("fldyMGTE7A")}="是",{public_pool_or_same_dept()}),"",'
