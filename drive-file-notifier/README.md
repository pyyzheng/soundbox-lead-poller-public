...
| 发送 | 同一文件夹多个小文件打成一个 ≤30MB zip；装不下则发该文件夹云盘链接（一条）；不同文件夹各发一条；单文件仍超 30MB 发文件云盘链接 |
| 忽略 | 标题含「禁止外发 / 内部 / 说明书 / 用户手册 / Installation Manual」等关键词不发群（见 `config.json` → `skipTitleKeywords`） |
...
- `staleRescanBudget`: 每轮允许的“陈旧目录重扫”预算，目录层级深时建议 `300` 以上，减少漏检。
