...
def get_feishu_token() -> str:
    ...
    max_retries = int(os.environ.get("FEISHU_TOKEN_RETRIES", "3"))
    last_exc = None
    for attempt in range(max_retries + 1):
        try:
            ...
            return token
                ...
                requests.exceptions.Timeout, RuntimeError) as exc:
            last_exc = exc
            if attempt < max_retries:
                delay = 2.0 * (2 ** attempt)
                log.warning("获取飞书 token 失败 %s，%.0fs 后重试 (%d/%d)", type(exc).__name__, delay, attempt + 1, max_retries)
                time.sleep(delay)
                continue
            raise
...
def feishu_api(method: str, url: str, token: str, max_retries: int = 3,
    ...
    """带 429/网络异常重试的飞书 API 请求。429/5xx/连接错误指数退避，最多 max_retries 次。"""
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt < max_retries:
                ...
                log.warning("飞书网络异常 %s，%.0fs 后重试 (%d/%d)", type(exc).__name__, delay, attempt + 1, max_retries)
                time.sleep(delay)
                continue
            raise
