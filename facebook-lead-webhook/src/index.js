...
var COUNTRY_SHORT_TO_WRITE = {"美国": "United States（美国）", "加拿大": "Canada（加拿大）", "澳大利亚": "Australia（澳大利亚）", "新西兰": "New Zealand（新西兰）", "德国": "Germany（德国）", "波兰": "Poland（波兰）", "日本": "Japan（日本）", "香港": "Hong Kong（香港）", "台湾": "Taiwan（台湾）", "英国": "United Kingdom（英国）", "韩国": "South Korea（韩国）", "新加坡": "Singapore（新加坡）", "泰国": "Thailand（泰国）", "沙特阿拉伯": "Saudi Arabia（沙特阿拉伯）", "以色列": "Israel（以色列）", "荷兰": "Netherlands（荷兰）", "阿联酋": "United Arab Emirates（阿联酋）", "法国": "France（法国）", "西班牙": "Spain（西班牙）", "马来西亚": "Malaysia（马来西亚）", "意大利": "Italy（意大利）", "印度尼西亚": "Indonesia（印度尼西亚）", "越南": "Vietnam（越南）", "斯洛文尼亚": "Slovenia（斯洛文尼亚）", "印度": "India（印度）", "菲律宾": "Philippines（菲律宾）", "爱尔兰": "Ireland（爱尔兰）", "爱沙尼亚": "Estonia（爱沙尼亚）", "奥地利": "Austria（奥地利）", "阿尔巴尼亚": "Albania（阿尔巴尼亚）", "阿尔及利亚": "Algeria（阿尔及利亚）", "阿根廷": "Argentina（阿根廷）", "白俄罗斯": "Belarus（白俄罗斯）", "保加利亚": "Bulgaria（保加利亚）", "比利时": "Belgium（比利时）", "伯利兹": "Belize（伯利兹）", "冰岛": "Iceland（冰岛）", "波斯尼亚和黑塞哥维那": "Bosnia and Herzegovina（波斯尼亚和黑塞哥维那）", "丹麦": "Denmark（丹麦）", "多明尼加共和国": "Dominican Republic（多明尼加共和国）", "芬兰": "Finland（芬兰）", "哥伦比亚": "Colombia（哥伦比亚）", "加纳": "Ghana（加纳）", "捷克": "Czechia（捷克）", "直布罗陀": "Gibraltar（直布罗陀）", "科索沃": "Kosovo（科索沃）", "肯尼亚": "Kenya（肯尼亚）", "克罗地亚": "Croatia（克罗地亚）", "拉脱维亚": "Latvia（拉脱维亚）", "立陶宛": "Lithuania（立陶宛）", "卢森堡": "Luxembourg（卢森堡）", "罗马尼亚": "Romania（罗马尼亚）", "马达加斯加": "Madagascar（马达加斯加）", "马尔代夫": "Maldives（马尔代夫）", "马耳他": "Malta（马耳他）", "挪威": "Norway（挪威）", "葡萄牙": "Portugal（葡萄牙）", "瑞典": "Sweden（瑞典）", "瑞士": "Switzerland（瑞士）", "俄罗斯": "Russia（俄罗斯）", "塞尔维亚": "Serbia（塞尔维亚）", "塞内加尔": "Senegal（塞内加尔）", "塞浦路斯": "Cyprus（塞浦路斯）", "斯洛伐克": "Slovakia（斯洛伐克）", "乌克兰": "Ukraine（乌克兰）", "希腊": "Greece（希腊）", "匈牙利": "Hungary（匈牙利）", "牙买加": "Jamaica（牙买加）", "泽西岛": "Jersey（泽西岛）", "黑山": "Montenegro（黑山）", "海地": "Haiti（海地）", "安哥拉": "Angola（安哥拉）", "澳门": "Macao（澳门）", "文莱": "Brunei（文莱）", "哈萨克斯坦": "Kazakhstan（哈萨克斯坦）", "吉尔吉斯斯坦": "Kyrgyzstan（吉尔吉斯斯坦）", "科威特": "Kuwait（科威特）", "黎巴嫩": "Lebanon（黎巴嫩）", "留尼汪": "Réunion（留尼汪）", "孟加拉国": "Bangladesh（孟加拉国）", "尼泊尔": "Nepal（尼泊尔）", "叙利亚": "Syria（叙利亚）", "索马里": "Somalia（索马里）", "坦桑尼亚": "Tanzania（坦桑尼亚）", "危地马拉": "Guatemala（危地马拉）", "乌兹别克斯坦": "Uzbekistan（乌兹别克斯坦）", "也门": "Yemen（也门）", "伊拉克": "Iraq（伊拉克）", "津巴布韦": "Zimbabwe（津巴布韦）", "无法识别": "Unrecognized（无法识别）", "无可用选项": "No options available（无可用选项）", "梵蒂冈": "Vatican City（梵蒂冈）", "北马其顿": "North Macedonia（北马其顿）", "巴基斯坦": "Pakistan（巴基斯坦）", "柬埔寨": "Cambodia（柬埔寨）", "智利": "Chile（智利）", "尼日利亚": "Nigeria（尼日利亚）", "南非": "South Africa（南非）", "蒙古": "Mongolia（蒙古）", "波多黎各": "Puerto Rico（波多黎各）", "埃及": "Egypt（埃及）", "阿曼": "Oman（阿曼）", "乌拉圭": "Uruguay（乌拉圭）", "秘鲁": "Peru（秘鲁）", "墨西哥": "Mexico（墨西哥）", "卡塔尔": "Qatar（卡塔尔）", "摩尔多瓦": "Moldova（摩尔多瓦）", "开曼群岛": "Cayman Islands（开曼群岛）", "苏丹": "Sudan（苏丹）", "博茨瓦纳": "Botswana（博茨瓦纳）", "毛里求斯": "Mauritius（毛里求斯）", "布基纳法索": "Burkina Faso（布基纳法索）", "格鲁吉亚": "Georgia（格鲁吉亚）", "巴拉圭": "Paraguay（巴拉圭）", "列支敦士登": "Liechtenstein（列支敦士登）", "圣马力诺": "San Marino（圣马力诺）", "巴拿马": "Panama（巴拿马）", "巴布亚新几内亚": "Papua New Guinea（巴布亚新几内亚）", "不丹": "Bhutan（不丹）", "东帝汶": "Timor-Leste（东帝汶）", "中非共和国": "Central African Republic（中非共和国）", "乌干达": "Uganda（乌干达）", "乍得": "Chad（乍得）", "亚美尼亚": "Armenia（亚美尼亚）", "伊朗": "Iran（伊朗）", "佛得角": "Cabo Verde（佛得角）", "冈比亚": "Gambia（冈比亚）", "几内亚": "Guinea（几内亚）", "几内亚比绍": "Guinea-Bissau（几内亚比绍）", "刚果": "Congo（刚果）", "利比亚": "Libya（利比亚）", "利比里亚": "Liberia（利比里亚）", "加蓬": "Gabon（加蓬）", "南苏丹": "South Sudan（南苏丹）", "卢旺达": "Rwanda（卢旺达）", "厄瓜多尔": "Ecuador（厄瓜多尔）", "厄立特里亚": "Eritrea（厄立特里亚）", "古巴": "Cuba（古巴）", "吉布提": "Djibouti（吉布提）", "哥斯达黎加": "Costa Rica（哥斯达黎加）", "喀麦隆": "Cameroon（喀麦隆）", "图瓦卢": "Tuvalu（图瓦卢）", "土库曼斯坦": "Turkmenistan（土库曼斯坦）", "土耳其": "Turkey（土耳其）", "圣卢西亚": "Saint Lucia（圣卢西亚）", "圣基茨和尼维斯": "Saint Kitts and Nevis（圣基茨和尼维斯）", "圣多美和普林西比": "Sao Tome and Principe（圣多美和普林西比）", "圣文森特和格林纳丁斯": "Saint Vincent and the Grenadines（圣文森特和格林纳丁斯）", "圭亚那": "Guyana（圭亚那）", "埃塞俄比亚": "Ethiopia（埃塞俄比亚）", "基里巴斯": "Kiribati（基里巴斯）", "塔吉克斯坦": "Tajikistan（塔吉克斯坦）", "塞拉利昂": "Sierra Leone（塞拉利昂）", "塞舌尔": "Seychelles（塞舌尔）", "多哥": "Togo（多哥）", "多米尼克": "Dominica（多米尼克）", "委内瑞拉": "Venezuela（委内瑞拉）", "安提瓜和巴布达": "Antigua and Barbuda（安提瓜和巴布达）", "安道尔": "Andorra（安道尔）", "密克罗尼西亚": "Micronesia（密克罗尼西亚）", "尼加拉瓜": "Nicaragua（尼加拉瓜）", "尼日尔": "Niger（尼日尔）", "巴勒斯坦": "Palestine（巴勒斯坦）", "巴哈马": "Bahamas（巴哈马）", "巴巴多斯": "Barbados（巴巴多斯）", "巴林": "Bahrain（巴林）", "巴西": "Brazil（巴西）", "布隆迪": "Burundi（布隆迪）", "帕劳": "Palau（帕劳）", "所罗门群岛": "Solomon Islands（所罗门群岛）", "摩洛哥": "Morocco（摩洛哥）", "摩纳哥": "Monaco（摩纳哥）", "斐济": "Fiji（斐济）", "斯威士兰": "Eswatini（斯威士兰）", "斯里兰卡": "Sri Lanka（斯里兰卡）", "朝鲜": "North Korea（朝鲜）", "根西岛": "Guernsey（根西岛）", "格林纳达": "Grenada（格林纳达）", "毛里塔尼亚": "Mauritania（毛里塔尼亚）", "汤加": "Tonga（汤加）", "法属圭亚那": "French Guiana（法属圭亚那）", "法罗群岛": "Faroe Islands（法罗群岛）", "洪都拉斯": "Honduras（洪都拉斯）", "特克斯和凯科斯群岛": "Turks and Caicos Islands（特克斯和凯科斯群岛）", "特立尼达和多巴哥": "Trinidad and Tobago（特立尼达和多巴哥）", "玻利维亚": "Bolivia（玻利维亚）", "瑙鲁": "Nauru（瑙鲁）", "瓦努阿图": "Vanuatu（瓦努阿图）", "百慕大群岛": "Bermuda（百慕大群岛）", "科摩罗": "Comoros（科摩罗）", "科特迪瓦": "Côte d'Ivoire（科特迪瓦）", "突尼斯": "Tunisia（突尼斯）", "约旦": "Jordan（约旦）", "纳米比亚": "Namibia（纳米比亚）", "缅甸": "Myanmar（缅甸）", "老挝": "Laos（老挝）", "苏里南": "Suriname（苏里南）", "荷属安的列斯": "Netherlands Antilles（荷属安的列斯）", "莫桑比克": "Mozambique（莫桑比克）", "莱索托": "Lesotho（莱索托）", "萨尔瓦多": "El Salvador（萨尔瓦多）", "萨摩亚": "Samoa（萨摩亚）", "贝宁": "Benin（贝宁）", "赞比亚": "Zambia（赞比亚）", "赤道几内亚": "Equatorial Guinea（赤道几内亚）", "阿塞拜疆": "Azerbaijan（阿塞拜疆）", "阿富汗": "Afghanistan（阿富汗）", "阿鲁巴": "Aruba（阿鲁巴）", "非洲": "Africa（非洲）", "马恩岛": "Isle of Man（马恩岛）", "马拉维": "Malawi（马拉维）", "马绍尔群岛": "Marshall Islands（马绍尔群岛）", "马里": "Mali（马里）", "中东": "Middle East（中东）", "中亚": "Central Asia（中亚）", "欧洲": "Europe（欧洲）", "南美": "South America（南美）", "中国": "China（中国）", "沙特": "Saudi Arabia（沙特）", "阿爾巴尼亞": "Albania（阿爾巴尼亞）", "中国香港": "Hong Kong（中国香港）", "大洋洲 法属波利尼西亚": "French Polynesia（大洋洲 法属波利尼西亚）", "荷属圣马丁": "Sint Maarten（荷属圣马丁）", "波多黎各 美国": "Puerto Rico（波多黎各 美国）", "印尼": "Indonesia（印度尼西亚）"};
function toWriteCountry(country) {
  const c = (country || "").trim();
  if (!c) return "";
  if (COUNTRY_SHORT_TO_WRITE[c]) return COUNTRY_SHORT_TO_WRITE[c];
  const m = c.match(/（([^）]+)）\s*$/);
  if (m && COUNTRY_SHORT_TO_WRITE[m[1]]) return COUNTRY_SHORT_TO_WRITE[m[1]];
  for (const v of Object.values(COUNTRY_SHORT_TO_WRITE)) {
    if (v === c) return c;
  }
  return c;
}
__name(toWriteCountry, "toWriteCountry");
...
var OPP_BASE = "Ktddb4mhtaixgYs9CIkcNnXFngh";
var OPP_TABLE = "tblz7klQPWxg9H15";
var OPP_FOLLOWUP_TABLE = "tbl5cYBxiF57LhbX";
var ROSTER_TABLE = "tblXq1rE7OQCrSgJ";
var FU_METHOD_OPTIONS = /* @__PURE__ */ new Set([
  "Email",
  "Phone Call",
  "Whatsapp",
  "Wechat",
...
function oppText(v) {
  ...
  if (Array.isArray(v)) {
    return v.map((x) => oppText(x)).filter(Boolean).join(" / ");
  }
  if (typeof v === "object") {
    if (v.value !== void 0) return oppText(v.value);
    return String(v.text || v.name || "").trim();
  }
  return "";
}
__name(oppText, "oppText");
function oppOption(v) {
  if (Array.isArray(v) && v.length) {
    const first = v[0];
    if (first && typeof first === "object") return String(first.text || first.name || "").trim();
    return oppText(first);
  }
  return oppText(v);
}
__name(oppOption, "oppOption");
function oppNum(v) {
  if (v == null || v === "") return null;
  if (typeof v === "number") return v;
  const s = oppText(v).replace(/,/g, "");
  const n = Number(s);
  return Number.isFinite(n) ? n : null;
}
__name(oppNum, "oppNum");
async function loadOppRoster(token, env) {
  const map = /* @__PURE__ */ new Map();
  const put = (alias, uid) => {
    const a = String(alias || "").trim();
    if (!a || !uid) return;
    map.set(a, uid);
    map.set(a.toLowerCase(), uid);
  };
  let pageToken = "";
  for (; ; ) {
    const url = `${FEISHU_BASE}/bitable/v1/apps/${env.FEISHU_APP_TOKEN}/tables/${ROSTER_TABLE}/records?page_size=500${pageToken ? `&page_token=${encodeURIComponent(pageToken)}` : ""}`;
    const res = await fetch(url, { headers: authHeaders(token) });
    const data = await res.json();
    if (data.code !== 0) throw new Error(`roster list failed: ${data.msg}`);
    for (const item of data.data?.items || []) {
      const f = item.fields || {};
      const name = oppText(f["业务名单"]);
      const users = f["对应业务"] || [];
      const user0 = Array.isArray(users) && users[0] ? users[0] : null;
      const uid = user0?.id || "";
      if (!uid) continue;
      put(name, uid);
      put(user0?.name || "", uid);
      put(user0?.en_name || "", uid);
      if (name === "Rita") put("Rita_USA", uid);
      if (name === "Jessica_US") put("Jessica", uid);
    }
...
__name(loadOppRoster, "loadOppRoster");
function resolveRosterUserIds(raw, roster) {
  if (!raw || !roster) return [];
  const parts = String(raw).split(/[,，、/;；|/]+/).map((p) => p.trim()).filter(Boolean);
  const ids = [];
  const seen = /* @__PURE__ */ new Set();
  for (const part of parts) {
    const uid = roster.get(part) || roster.get(part.toLowerCase());
    if (uid && !seen.has(uid)) {
      seen.add(uid);
      ids.push(uid);
    }
...
__name(resolveRosterUserIds, "resolveRosterUserIds");
function buildOppFields(fields, roster) {
  const level = oppOption(fields["🌟Case Level / 线索分级"]);
  if (![...OPP_SYNC_LEVELS].some((x) => x === level || x.trim() === level.trim())) return null;
  const clueId = oppText(fields["Clue ID"]);
  if (!clueId) return null;
  const contact = oppText(fields["Customer Name（客户名称）"]);
  const unit = oppText(fields["Customer Name 客戶單位"]);
  const channel = oppOption(fields["Channels（渠道）"]);
  const product = oppOption(fields["Product Categories（产品大类）"]);
  const assignee = oppText(fields["The final assigned salesperson（最终分配的业务员）"]);
  const nextStep = oppText(fields["Next Step"]) || oppText(fields["Quick Follow-up / 快捷跟进"]);
  const value = oppNum(fields["🌟value of the lead（线索预估价值）"]);
  const dealAmt = oppNum(fields["Deal Amount / 成交金额"]);
  const stage = OPP_STAGE_MAP[level] || OPP_STAGE_MAP[level.trim()] || null;
  const out = {
    "源线索ID": clueId,
    "源Case Level": level.trim(),
    "客户名称": unit || contact,
    "联系人姓名": contact,
    "客户邮箱": oppText(fields["Email（客户邮箱）"]),
    "联系电话": oppText(fields["Phone（客户电话）"]),
    "公司地址": oppText(fields["单位地址 Address"]),
    "客户类型": oppOption(fields["Customer type"]),
    "商机描述": oppText(fields["Enquiry details（询盘内容）"]),
    "一句话进展": nextStep,
    "商机阶段": stage,
    "商机来源": OPP_CHANNEL_SOURCE[channel] || "其他"
  };
  if (OPP_PRODUCT_ALLOWED.has(product)) out["意向产品"] = [product];
  if (value != null) out["预计成交金额（元）"] = value;
  if (dealAmt != null) out["实际成交金额（元）"] = String(dealAmt);
  if (assignee && roster.get(assignee)) out["业务员"] = [{ id: roster.get(assignee) }];
  for (const k of Object.keys(out)) {
    if (out[k] == null || out[k] === "" || Array.isArray(out[k]) && out[k].length === 0) delete out[k];
  }
  return out;
}
__name(buildOppFields, "buildOppFields");
async function findOppByClueId(token, clueId) {
  const res = await fetch(
    `${FEISHU_BASE}/bitable/v1/apps/${OPP_BASE}/tables/${OPP_TABLE}/records/search`,
    {
      method: "POST",
      headers: authHeaders(token),
      body: JSON.stringify({
        filter: {
          conjunction: "and",
          conditions: [{ field_name: "源线索ID", operator: "is", value: [clueId] }]
        },
        page_size: 5
      })
    }
  ...
  const data = await res.json();
  if (data.code !== 0) throw new Error(`opp search failed: ${data.msg}`);
  const item = (data.data?.items || [])[0];
  return item?.record_id || item?.id || "";
}
__name(findOppByClueId, "findOppByClueId");
async function upsertOpportunity(token, fields) {
  const clueId = fields["源线索ID"];
  const existing = await findOppByClueId(token, clueId);
  if (existing) {
    const res = await fetch(
      `${FEISHU_BASE}/bitable/v1/apps/${OPP_BASE}/tables/${OPP_TABLE}/records/${existing}`,
      { method: "PUT", headers: authHeaders(token), body: JSON.stringify({ fields }) }
    );
    const data = await res.json();
    if (data.code !== 0) throw new Error(`opp update failed: ${data.msg}`);
    return { action: "updated", record_id: existing, clue_id: clueId };
  }
  const res = await fetch(
    `${FEISHU_BASE}/bitable/v1/apps/${OPP_BASE}/tables/${OPP_TABLE}/records`,
    { method: "POST", headers: authHeaders(token), body: JSON.stringify({ fields }) }
  );
  const data = await res.json();
  if (data.code !== 0) throw new Error(`opp create failed: ${data.msg}`);
  return { action: "created", record_id: data.data.record.record_id, clue_id: clueId };
}
__name(upsertOpportunity, "upsertOpportunity");
async function handleSyncOpportunity(request, env) {
  const body = await request.json().catch(() => ({}));
  const recordId = String(body.record_id || body.lead_record_id || "").trim();
  if (!recordId) {
    return Response.json({ ok: false, error: "record_id required" }, { status: 400 });
  }
  const token = await getTenantToken(env);
  const getRes = await fetch(`${tableUrl(env)}/${recordId}`, { headers: authHeaders(token) });
  const getData = await getRes.json();
  if (getData.code !== 0 || !getData.data?.record) {
    return Response.json({ ok: false, error: `lead not found: ${getData.msg || recordId}` }, { status: 404 });
  }
  const roster = await loadOppRoster(token, env);
  const mapped = buildOppFields(getData.data.record.fields || {}, roster);
  if (!mapped) {
    return Response.json({ ok: true, action: "skipped", reason: "case_level_not_a2_a5", record_id: recordId });
  }
  const result = await upsertOpportunity(token, mapped);
  let followupSync = null;
  try {
    followupSync = await syncFollowupsForLead(token, env, recordId, mapped["源线索ID"], roster);
  } catch (err) {
    console.error(`[sync-opportunity] followup backfill failed: ${err.message}`);
    followupSync = { ok: false, error: err.message };
  }
  return Response.json({ ok: true, ...result, followup_sync: followupSync });
}
__name(handleSyncOpportunity, "handleSyncOpportunity");
function fuLinkIds(v) {
  const out = [];
  const walk = (obj) => {
    if (!obj) return;
    if (typeof obj === "string") {
      if (obj.startsWith("rec")) out.push(obj);
      return;
    }
    ...
    if (typeof obj === "object") {
      for (const key of ["link_record_ids", "record_ids"]) {
        if (Array.isArray(obj[key])) out.push(...obj[key].map(String));
      }
      if (obj.record_id) out.push(String(obj.record_id));
      if (obj.id && String(obj.id).startsWith("rec")) out.push(String(obj.id));
    }
  };
  walk(v);
  return [...new Set(out)];
}
__name(fuLinkIds, "fuLinkIds");
function fuMapMethod(raw) {
  if (!raw) return "";
  if (FU_METHOD_OPTIONS.has(raw)) return raw;
  if (FU_METHOD_FALLBACK[raw]) return FU_METHOD_FALLBACK[raw];
  const first = String(raw).split(/[,/]/)[0].trim();
  return FU_METHOD_OPTIONS.has(first) ? first : "";
}
__name(fuMapMethod, "fuMapMethod");
function fuDatetimeMs(v) {
  if (v == null || v === "") return null;
  if (typeof v === "number") return v;
  const s = oppText(v);
  if (!s) return null;
...
__name(fuDatetimeMs, "fuDatetimeMs");
function buildOppFollowupFields(fields, { oppId, roster, clueId }) {
  if (fields["已删除"] === true) return null;
  const fuId = oppText(fields["Follow-up ID"]);
  if (!fuId || !oppId) return null;
  let details = oppText(fields["Follow-up Details"]);
  const attendees = oppText(fields["参会人 Attendees"]);
  const attendeeIds = resolveRosterUserIds(attendees, roster);
  if (attendees) {
    const unresolved = String(attendees).split(/[,，、/;；|/]+/).map((p) => p.trim()).filter(Boolean).filter((p) => !(roster?.get(p) || roster?.get(p.toLowerCase())));
    if (unresolved.length) {
      const tip = unresolved.join("、");
      details = details ? `\u3010\u672A\u8BC6\u522B\u53C2\u4F1A\u4EBA\u3011${tip}\n${details}` : `\u3010\u672A\u8BC6\u522B\u53C2\u4F1A\u4EBA\u3011${tip}`;
    }
  }
  const nextStep = oppOption(fields["Next Step"]);
  const method = fuMapMethod(oppOption(fields["Contact Method"]));
  const ts = fuDatetimeMs(fields["Follow-up Time"]);
  const customer = oppText(fields["Customer Name"]);
  const titleParts = [];
  if (ts != null) {
    const d = new Date(ts);
    titleParts.push(`${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`);
  }
  if (nextStep) titleParts.push(String(nextStep).slice(0, 40));
  if (customer) titleParts.push(String(customer).slice(0, 20));
  const followTitle = titleParts.join(" · ") || fuId;
  const out = {
    "源Follow-up ID": fuId,
    "源线索ID": clueId || "",
    "跟进标题": followTitle,
    "关联商机": [oppId],
    "客户名称": customer,
    "地點 Location": oppText(fields["地点 Location"]),
    "意向产品": oppText(fields["意向产品 Products"]),
    "合作區域 Region": oppText(fields["合作区域 Region"]),
    "報價折扣 Quote/Disc.": oppText(fields["报价折扣 Quote/Disc"]),
    "Follow-up Records/跟进记录.文本": details,
    "贈送禮品 Gifts": oppText(fields["赠送礼品 Gifts"]),
    "推進計劃 Next Step": nextStep,
    "One-line Progress/一句话进展": nextStep
  };
  if (attendees) out["参会人文本 Attendees"] = attendees;
  if (attendeeIds.length) out["參會人 Attendees"] = attendeeIds.map((id) => ({ id }));
  if (method) out["方式 Method"] = method;
  if (ts != null) out["時間 Date"] = ts;
  const salesUsers = fields["Salesperson"];
  let salesIds = [];
  if (Array.isArray(salesUsers)) {
    salesIds = salesUsers.map((u) => u?.id).filter(Boolean);
  } else if (salesUsers && typeof salesUsers === "object") {
    if (Array.isArray(salesUsers.users)) {
      salesIds = salesUsers.users.map((u) => u?.id).filter(Boolean);
    } else if (salesUsers.id) {
      salesIds = [salesUsers.id];
    }
  }
  if (salesIds.length) {
    out["业务员"] = [{ id: salesIds[0] }];
  } else {
    const salesName = oppText(salesUsers);
    const uid = salesName && roster ? roster.get(salesName) || roster.get(salesName.toLowerCase()) : "";
    if (uid) out["业务员"] = [{ id: uid }];
  }
  for (const k of Object.keys(out)) {
    if (out[k] == null || out[k] === "" || Array.isArray(out[k]) && out[k].length === 0) delete out[k];
  }
...
__name(buildOppFollowupFields, "buildOppFollowupFields");
async function findOppFollowupBySourceId(token, fuId) {
  const res = await fetch(
    `${FEISHU_BASE}/bitable/v1/apps/${OPP_BASE}/tables/${OPP_FOLLOWUP_TABLE}/records/search`,
    {
      method: "POST",
      headers: authHeaders(token),
      body: JSON.stringify({
        filter: {
          conjunction: "and",
          conditions: [{ field_name: "源Follow-up ID", operator: "is", value: [fuId] }]
        },
        page_size: 5
      })
    }
  ...
  const data = await res.json();
  if (data.code !== 0) throw new Error(`opp followup search failed: ${data.msg}`);
  const item = (data.data?.items || [])[0];
  return item?.record_id || item?.id || "";
}
__name(findOppFollowupBySourceId, "findOppFollowupBySourceId");
async function upsertOppFollowup(token, fields) {
  const fuId = fields["源Follow-up ID"];
  const existing = await findOppFollowupBySourceId(token, fuId);
  if (existing) {
    const res = await fetch(
      `${FEISHU_BASE}/bitable/v1/apps/${OPP_BASE}/tables/${OPP_FOLLOWUP_TABLE}/records/${existing}`,
      { method: "PUT", headers: authHeaders(token), body: JSON.stringify({ fields }) }
    );
    const data = await res.json();
    if (data.code !== 0) throw new Error(`opp followup update failed: ${data.msg}`);
    return { action: "updated", record_id: existing, followup_id: fuId };
  }
  const res = await fetch(
    `${FEISHU_BASE}/bitable/v1/apps/${OPP_BASE}/tables/${OPP_FOLLOWUP_TABLE}/records`,
    { method: "POST", headers: authHeaders(token), body: JSON.stringify({ fields }) }
  );
  const data = await res.json();
  if (data.code !== 0) throw new Error(`opp followup create failed: ${data.msg}`);
  return { action: "created", record_id: data.data.record.record_id, followup_id: fuId };
}
__name(upsertOppFollowup, "upsertOppFollowup");
async function resolveClueIdFromLead(token, env, leadRecordId) {
  const res = await fetch(`${tableUrl(env)}/${leadRecordId}`, { headers: authHeaders(token) });
  const data = await res.json();
  if (data.code !== 0 || !data.data?.record) return "";
  return oppText(data.data.record.fields?.["Clue ID"]);
}
__name(resolveClueIdFromLead, "resolveClueIdFromLead");
async function listFollowupsForLead(token, env, leadRecordId) {
  const items = [];
  let pageToken = void 0;
  do {
    const body = {
    ...
    const data = await bitableSearch(token, env, FOLLOWUP_TABLE_ID, body);
    if (data.code !== 0) {
      throw new Error(`list followups for opp sync failed: code=${data.code} msg=${data.msg}`);
    }
...
__name(listFollowupsForLead, "listFollowupsForLead");
async function syncFollowupsForLead(token, env, leadRecordId, clueId, roster) {
  const resolvedClueId = clueId || await resolveClueIdFromLead(token, env, leadRecordId);
  const oppId = await findOppByClueId(token, resolvedClueId);
  if (!oppId) return { ok: true, action: "skipped", reason: "no_opportunity", synced: 0 };
  const followups = await listFollowupsForLead(token, env, leadRecordId);
  const stats = { created: 0, updated: 0, skipped: 0, error: 0 };
  for (const rec of followups) {
    try {
      const mapped = buildOppFollowupFields(rec.fields || {}, { oppId, roster, clueId: resolvedClueId });
      if (!mapped) {
        stats.skipped += 1;
        continue;
      }
      const result = await upsertOppFollowup(token, mapped);
      stats[result.action] = (stats[result.action] || 0) + 1;
    } catch (err) {
      stats.error += 1;
      console.error(`[sync-followup] lead backfill item failed: ${err.message}`);
    }
  }
  return { ok: true, action: "backfilled", opp_id: oppId, total: followups.length, ...stats };
}
__name(syncFollowupsForLead, "syncFollowupsForLead");
async function handleSyncFollowupToOpp(request, env) {
  const body = await request.json().catch(() => ({}));
  const recordId = String(body.record_id || body.followup_record_id || "").trim();
  if (!recordId) {
    return Response.json({ ok: false, error: "record_id required" }, { status: 400 });
  }
  const token = await getTenantToken(env);
  const getRes = await fetch(
    `${FEISHU_BASE}/bitable/v1/apps/${env.FEISHU_APP_TOKEN}/tables/${FOLLOWUP_TABLE_ID}/records/${recordId}`,
    { headers: authHeaders(token) }
  );
  const getData = await getRes.json();
  if (getData.code !== 0 || !getData.data?.record) {
    return Response.json({ ok: false, error: `followup not found: ${getData.msg || recordId}` }, { status: 404 });
  }
  const fields = getData.data.record.fields || {};
  const related = fuLinkIds(fields["Related Lead"]);
  if (!related.length) {
    return Response.json({ ok: true, action: "skipped", reason: "no_related_lead", record_id: recordId });
  }
  let clueId = "";
  let leadId = "";
  for (const rid of related) {
    clueId = await resolveClueIdFromLead(token, env, rid);
    if (clueId) {
      leadId = rid;
      break;
    }
  ...
  if (!clueId) {
    return Response.json({ ok: true, action: "skipped", reason: "no_clue_id", record_id: recordId });
  }
  const oppId = await findOppByClueId(token, clueId);
  if (!oppId) {
    return Response.json({
      ok: true,
      action: "skipped",
      reason: "lead_not_in_opportunity",
      record_id: recordId,
      clue_id: clueId,
      lead_id: leadId
    });
  ...
  const roster = await loadOppRoster(token, env);
  const mapped = buildOppFollowupFields(fields, { oppId, roster, clueId });
  if (!mapped) {
    return Response.json({ ok: true, action: "skipped", reason: "deleted_or_empty", record_id: recordId });
  }
  const result = await upsertOppFollowup(token, mapped);
  return Response.json({ ok: true, ...result, clue_id: clueId, opp_id: oppId });
}
__name(handleSyncFollowupToOpp, "handleSyncFollowupToOpp");
...
async function createRecord(token, parsed, env) {
  ...
  if (parsed.country) fields[FEISHU_FIELDS.COUNTRY] = toWriteCountry(parsed.country);
...
var index_default = {
  async fetch(request, env, ctx) {
    ...
    if (request.method === "POST" && url.pathname === "/sync-opportunity") {
      const auth = await requireAuth(request, env, { allowBodyToken: true });
      if (!auth.ok) return auth.response;
      try {
        return await handleSyncOpportunity(request, env);
      } catch (err) {
        console.error(`[sync-opportunity] ${err.message}`);
        return Response.json({ ok: false, error: err.message }, { status: 500 });
      }
    }
    if (request.method === "POST" && url.pathname === "/sync-followup-to-opp") {
      const auth = await requireAuth(request, env, { allowBodyToken: true });
      if (!auth.ok) return auth.response;
      try {
        return await handleSyncFollowupToOpp(request, env);
      } catch (err) {
        console.error(`[sync-followup-to-opp] ${err.message}`);
        return Response.json({ ok: false, error: err.message }, { status: 500 });
      }
    }
